...
test("job-alert email delivery is an explicit professional opt-in", async () => {
  const migration = await readProjectFile(
    "supabase/migrations/20260828073013_job_alert_email_delivery.sql",
  )
  const emailDelivery = await readProjectFile("src/lib/jobs/job-alert-email.ts")

  assert.match(migration, /email_alerts_enabled boolean not null default false/)
  assert.match(emailDelivery, /\.eq\("alerts_enabled", true\)/)
  assert.match(emailDelivery, /\.eq\("email_alerts_enabled", true\)/)
  assert.match(emailDelivery, /profile\.account_type === "professional"/)
})
