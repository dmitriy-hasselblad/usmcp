...
test("sample jobs remain visibly distinct from live marketplace opportunities", async () => {
  ...
  assert.match(jobsPage, /const activeJobs = \[\.\.\.usaJobs, \.\.\.officialAtsJobs, \.\.\.liveJobs\]/)
  assert.match(jobsPage, /const allJobs = showPreviews \? \[\.\.\.activeJobs, \.\.\.featuredJobs\] : activeJobs/)
  ...
  assert.match(jobCard, /Federal opportunity · USAJOBS/)
  assert.match(jobCard, /Official employer · \$\{officialAtsProviderName\(job\.source\)\}/)
  assert.match(jobCard, /View & apply on \{job\.source === "usajobs" \? "USAJOBS" : job\.sourceName \?\? "employer site"\}/)
