...
export function MessageAttachmentUpload({ applicationId, userId }: { applicationId: string; userId: string }) {
  ...
  async function upload(event: FormEvent<HTMLFormElement>) {
    ...
    try { await registerApplicationMessageAttachment(data) } catch { await supabase.storage.from(applicationMessageAttachmentsBucket).remove([storagePath]); setError("The attachment could not be saved."); setPending(false) }
  }
