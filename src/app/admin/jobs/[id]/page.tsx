...
import { changeJobModeration, permanentlyDeleteJob } from "@/app/admin/jobs/actions"
...
function DeleteJobForm({ id }: { id: string }) { return <Card className="border-red-200 bg-red-50"><CardContent className="p-6"><Trash2 className="size-6 text-red-700" /><h2 className="mt-4 text-lg font-semibold text-red-950">Permanent removal</h2><p className="mt-2 text-sm leading-6 text-red-900">This permanently removes the job. Jobs with applications are protected and cannot be removed.</p><form action={permanentlyDeleteJob} className="mt-5"><input name="jobId" type="hidden" value={id} /><label className="grid gap-2 text-sm font-medium text-red-950">Type DELETE to confirm<Input autoComplete="off" name="confirmation" required /></label><label className="mt-3 flex items-start gap-2 text-xs leading-5 text-red-900"><input className="mt-1 size-4" name="confirmed" required type="checkbox" />I understand this cannot be undone.</label><Button className="mt-4 w-full" type="submit" variant="destructive"><Trash2 /> Delete job permanently</Button></form></CardContent></Card> }
