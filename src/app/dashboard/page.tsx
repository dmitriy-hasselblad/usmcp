...
export const metadata: Metadata = {
  title: "Dashboard",
  description: "Your secure SM VIA account workspace.",
}
    ...
    <ProfessionalDashboardShell active="overview" email={email}>
      ...
      <Card className="mt-6 bg-white">
        <CardContent className="p-6">
          ...
          {recommendations.length ? <div className="mt-5 grid gap-3">{recommendations.map((job) => <Link className="rounded-xl border p-4 transition-colors hover:bg-muted/40" href={`/jobs/${job.slug}`} key={job.slug}><p className="font-semibold">{job.title}</p><p className="mt-1 text-sm text-muted-foreground">{job.employer} · {job.location}</p><p className="mt-2 text-xs font-medium text-primary">{job.matchReasons.join(" · ")}</p></Link>)}</div> : <p className="mt-5 text-sm text-muted-foreground">{roleProfile ? "No close matches are available right now. Browse all live jobs or check back as employers publish new opportunities." : "Add your profession, specialty, and state to receive job recommendations."}</p>}
        </CardContent>
      </Card>
