...
import { parseResumeContent } from "@/lib/resume/types"
import { getResumeExportAccess } from "@/lib/resume/export-access"

export const metadata: Metadata = { title: "Edit résumé", description: "Build a private U.S. healthcare résumé." }
...
export default async function ResumePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string | string[]; success?: string | string[] }> }) {
  ...
  const { data: resume } = await identity.supabase.from("professional_resumes").select("id, title, content").eq("id", id).eq("user_id", identity.userId).maybeSingle()
  if (!resume) notFound()
  ...
  return <ProfessionalDashboardShell active="resumes" email={identity.email}>
    ...
    <ResumeEditor exportAccess={getResumeExportAccess()} initialContent={parseResumeContent(resume.content)} resumeId={resume.id} title={resume.title}/>
