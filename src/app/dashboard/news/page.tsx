...
export default async function NewsPage({ searchParams }: { searchParams: Promise<{ success?: string | string[]; error?: string | string[] }> }) {
  ...
  return <EmployerDashboardShell active="news" email={workspace.email} organizationName={workspace.organization.name}>
    <div className="flex flex-wrap items-start justify-between gap-4"><EmployerPageHeader eyebrow="Organization publishing" title="News & insights" description="Share organization updates, healthcare insights, and stories with the SMVIA community." />{canManageJobs(workspace.membership.role) && <Button asChild><Link href="/dashboard/news/new">Create article</Link></Button>}</div>
